...
describe('natural improvement', () => {
  describe('natural abilities selection', () => {
    ...
    beforeEach(() => {
      ...
      naturalAbilitiesSelection = new NaturalAbilitiesSelection(secondaryAbilities)
    })
  ...
  describe('natural bonus', () => {
    ...
    beforeEach(() => {
      ...
      naturalBonusSelection = new NaturalBonusSelection(characterCreator.characteristicsSelection, characterCreator.secondaryAbilities)
    })
    describe('select a natural bonus', () => {
      test('when select a physical Ability then i can see the Ability than i select', () => {
        naturalBonusSelection.selectPhysicalAbility('feats of strength')
        expect(naturalBonusSelection.selectedPhysicalAbility).toBe('feats of strength')
      })
      test('when select a non physical ability then i get a error', () => {
        expect(
          () => naturalBonusSelection.selectPhysicalAbility('persuasion')).toThrow('persuasion is not depends on a physical characteristic')
      })
      test('when i select a psychic ability i can see the Ability than i select', () => {
        naturalBonusSelection.selectPsychicAbility('persuasion')
        expect(naturalBonusSelection.selectedPsychicAbility).toBe('persuasion')
      })
      test('when select a non psychic ability then i get  a error', () => {
        expect(
          () => {
            naturalBonusSelection.selectPsychicAbility('feats of strength').toThrow('feats of strength is not depends on a psychic characteristic')
          })
      })
      test('when select a physical ability, then the ability have a bonus', () => {
        naturalBonusSelection.selectPhysicalAbility('jump')
        const bonuses = characterCreator.secondaryAbilities.get('jump').bonuses
        const bonus = { reason: 'natural bonus', value: -40 }
      ...
      test('when select a physical ability and then change the value of a characteristic then the bonus change', () => {
        naturalBonusSelection.selectPhysicalAbility('jump')
        const beforeBonus = characterCreator.secondaryAbilities.get('jump').bonuses.find(x => x.reason === 'natural bonus')
        characterCreator.characteristicsSelection.generatePoints(5).expendPointsTo('agility', 9)
        const afterBonus = characterCreator.secondaryAbilities.get('jump').bonuses.find(x => x.reason === 'natural bonus')
        expect(beforeBonus.value).not.toBe(afterBonus.value)
      })
      test('when i change the selection the bonus is removed from the last ability', () => {
        naturalBonusSelection.selectPhysicalAbility('jump')
        naturalBonusSelection.selectPhysicalAbility('climb')
        expect(characterCreator.secondaryAbilities.get('jump').bonuses).not.toContainEqual({ reason: 'natural bonus', value: -40 })
      })
    })
  })
})
